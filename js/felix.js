...
var resize = function() {
	...
	felixPos.x = Math.round(window.innerWidth*.8-felixPos.w) * window.devicePixelRatio;
	felixPos.w = felixPos.w * window.devicePixelRatio;
	felixPos.y = felixPos.y * window.devicePixelRatio;
...
var drawStars = function() {
	...
	for (i = 0; i < stars.length; i++) {
		...
		if(now > stars[i].lifetime){
			if(stars[i].opac > 0){
				...
				stars[i].radius = .1 + Math.random() * 3.8;
	...
	/*	ctx.fillStyle = 'rgba(8,247,167,'+stars[i].opac+')';*/
		ctx.fillStyle = 'rgba(255,242,0,'+stars[i].opac+')';
...
/*	ctx.drawImage(felix, felixPos.x+deltaX*.4, felixPos.y+deltaY*.4, felixPos.w, felixPos.w);*/
